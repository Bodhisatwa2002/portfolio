    ...
    <div id="work" className="mt-10 relative">
      ...
      <Div className="mb-10">
        <div className="flex text-center justify-center gap-2 md:gap-0 md:flex-col text-[40px] md:text-[90px] 2xl:text-[120px] leading-[40px] md:leading-[95px] 2xl:leading-[123px] font-oswald uppercase text-gradient mb-2">
          <style jsx>{`
            .text-gradient {
              background: linear-gradient(to right, #eeaed2 0%, #94bbe9 100%);
        ...
        {projectData.map((item, index) => {
          return (
            <Div key={index} className="flex flex-col gap-4 cursor-pointer">
              <div
                className="bg-black rounded-[20px] aspect-video overflow-hidden"
                onClick={() => openModal(item)}
              >
                <img
                  src={item.image}
                  className="transition-transform hover:-translate-y-[50%] hover:duration-2500 ease-linear"
                />
              </div>
              <div
                className="text-[20px] 2xl:text-[24px] text-[#CCCCCC] text-center"
                onClick={() => openModal(item)}
              >
                {item.name}
              </div>
